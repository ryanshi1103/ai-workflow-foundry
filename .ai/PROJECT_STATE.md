...
- [x] 完成 GitHub 个人简介、公开仓库英文简介与 3 个作品 Logo 的线上核验
- [x] 发布第二批 GitHub 作品：安全照片归档、通用姓名牌生成器与 10 个 Confera 媒体 Skill
- [x] 将核心工具提取为不含本机运维资料的 `ai-workspace-manager` 公共快照，108 项回归检查通过
- [x] 全量复核活跃项目并建立产品线边界：平台、扩展包、垂直应用、专项工具与私有案例分开管理
- [x] 发布 `ai-workflow-foundry` 旗舰平台：组件目录 CLI、通用契约、架构/路线图、Logo、15 项基础测试和 108 项底座检查
- [x] 为 4 个关联公开仓库建立本地规范克隆、双向产品说明和 `flowfoundry-ai` 主题
- [x] 为反馈分析公开仓库补齐 GitHub Actions，Ruff 与 90 项测试纳入线上验证
- [x] 重构 GitHub 主页：AI 工作流为主线，Android 工具降为专项作品，并更新账号简介
- [x] 将 cc 项目选择器收敛为主项目与托管/归档两层视图，并为托管仓库增加安全自动快进更新
- [x] 将四个公开 AI 工作流组件连同 Git 历史物理整合进 FlowFoundry monorepo
- [x] 清理已归并/可重建的隔离区副本，并将无远端备份的 `codex-claude` 旧构建转入恢复隔离区

### 待完成
- [ ] systemd timer 实际运行验证
- [ ] 2026-08-01 照片归档隔离项达到保留期后复核
...
2026-08-04 — 清理无价值/重复副本，保留照片与独有网络资料的安全边界
