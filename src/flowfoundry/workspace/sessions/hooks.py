...
def _get_hook_logger():
    ...
    if _HOOK_LOGGER is not None:
        return _HOOK_LOGGER
    logger = logging.getLogger("ai-project-manager.hooks")
    ...
    if logger.handlers:
        ...
        return logger
    try:
        d = Path.home() / ".local/state/ai-project-manager/logs"
        d.mkdir(parents=True, exist_ok=True)
        h = logging.FileHandler(d / "hook-events.log", encoding="utf-8")
...
def handle_hook_event():
    ...
    try:
        _hook_log.info("Hook invoked: pid=%s cwd=%s", os.getpid(), os.getcwd())
    ...
    if not project_dir:
        _hook_log.warning(
            "Could not find project: cwd=%s session_id=%s project_env_set=%s",
            cwd,
            session_id,
            bool(os.environ.get("AI_PROJECT_MANAGER_PROJECT")),
        )
        sys.exit(0)
    ...
    **IMPORTANT**: This hook MUST NOT create new top-level projects.
    ...
    if session_dir:
        ...
        if session_meta:
            ...
            session_meta["permission_mode"] = permission_mode or session_meta.get(
                "permission_mode", ""
            )
...
def _sync_transcript(project_dir: Path, session_dir: Path, tool: str) -> None:
    ...
    hash_file.write_text(
        f"{transcript_hash}  transcript.redacted.jsonl\n", encoding="utf-8"
    )
...
def _record_event(session_dir: Path, event_type: str, event_data: dict) -> None:
    ...
    record = {
        ...
        "event": event_type,
        "data": {k: v for k, v in event_data.items() if k not in ("raw", "full_event")},
    }
...
HOOK_ENTRY_SCRIPT = str(Path.home() / ".local" / "libexec" / "ai-project-manager-hook")
