...
def _normalize_codex_event(event: dict, line_num: int) -> dict | None:
    ...
    if payload_type == "agent_message":
        result = _message_event(
            "assistant", payload.get("message", ""), event, line_num
        )
        result["phase"] = payload.get("phase", "")
        return result
    if payload_type == "task_complete" and payload.get("last_agent_message"):
        result = _message_event(
            "assistant", payload["last_agent_message"], event, line_num
        )
        result["phase"] = "final_answer"
        return result
    if event.get("type") in ("event_msg", "response_item") and payload_type not in (
        "message",
    ):
        return None
    ...
    event_type = (
        event.get("type") or event.get("role") or event.get("event") or "unknown"
    )
    ...
    # Timestamp
    normalized["timestamp"] = event.get(
        "timestamp", event.get("created_at", event.get("time", ""))
    )
...
def extract_accomplishments(events: list[dict]) -> list[str]:
    ...
    for evt in events:
        if evt.get("role") in ("assistant", "ai", "model", "codex"):
            ...
            if any(
                marker in content
                for marker in ("已创建", "已完成", "已写入", "成功创建")
            ):
