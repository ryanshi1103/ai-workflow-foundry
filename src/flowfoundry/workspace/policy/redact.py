...
REDACTION_RULES = [
    ...
    (
        re.compile(
            r'(?:api[_-]?key|apikey)\s*[:=]\s*["\']?([\w\-_\.]{16,})["\']?',
            re.IGNORECASE,
        ),
        lambda m: m.group(0).replace(m.group(1), "[REDACTED_API_KEY]"),
    ),
    # OpenAI sk- keys
    (re.compile(r"\b(sk-(?:proj-)?[A-Za-z0-9_\-]{20,})\b"), "[REDACTED_API_KEY]"),
    # Anthropic API keys (sk-ant-)
    (re.compile(r"\b(sk-ant-[A-Za-z0-9_\-]{20,})\b"), "[REDACTED_API_KEY]"),
    # DeepSeek API keys
    (re.compile(r"\b(ds-[A-Za-z0-9_\-]{16,})\b"), "[REDACTED_API_KEY]"),
    ...
    (re.compile(r"://[^:@/\s]+:[^:@/\s]+@"), "://[REDACTED_CREDENTIALS]@"),
    # Connection strings with passwords
    (
        re.compile(
            r"(mongodb|mysql|postgresql|redis|jdbc)://[^:@\s]+:[^:@\s]+@([^\s]+)"
        ),
    ...
    (
        re.compile(
            r'(ANTHROPIC_API_KEY|OPENAI_API_KEY|DEEPSEEK_API_KEY|API_KEY|AUTH_TOKEN|SECRET_KEY)\s*=\s*["\']?([\w\-_\.]{12,})["\']?'
        ),
    ...
    (
        re.compile(
            r'(cookie|session)\s*[:=]\s*["\']([\w\-_\.=]{20,})["\']', re.IGNORECASE
        ),
    ...
    (
        re.compile(
            r'(access[_-]?token|auth[_-]?token|refresh[_-]?token)\s*[:=]\s*["\']?([\w\-_\.]{16,})["\']?',
            re.IGNORECASE,
        ),
        lambda m: f"{m.group(1)}=[REDACTED_TOKEN]",
    ),
...
def scan_for_secrets(text: str) -> list[str]:
    ...
    checks = [
        ...
        (
            re.compile(
                r"(Authorization\s*:\s*Bearer\s+)[\w\-_\.=]{16,}", re.IGNORECASE
            ),
        ...
        (re.compile(r"://[^:@/\s]+:[^:@/\s]+@"), "URL with embedded credentials found"),
    ]
