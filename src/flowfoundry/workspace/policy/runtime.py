...
SENSITIVE_PATTERNS = [
    (
        r'(api_key|apikey|api-key)\s*[:=]\s*["\']?[\w\-_\.]{20,}["\']?',
        "[REDACTED_API_KEY]",
    ),
    (r"(Bearer\s+)[\w\-_\.=]{20,}", r"\1[REDACTED_TOKEN]"),
    (r"(Authorization:\s*Bearer\s+)[\w\-_\.=]{20,}", r"\1[REDACTED_TOKEN]"),
    (
        r'(ANTHROPIC_API_KEY|OPENAI_API_KEY|DEEPSEEK_API_KEY)\s*=\s*["\']?[\w\-_\.]{20,}["\']?',
...
def file_lock(lock_path: Path, timeout: float = 30.0):
    ...
    try:
        ...
        try:
            ...
            logging.getLogger("ai-project-manager").warning(
                "Failed to unlock %s", lock_path
            )
        os.close(fd)
    ...
    for d in path_dirs:
        ...
        if candidate.is_file() and os.access(candidate, os.X_OK):
            ...
            try:
                ...
                if (
                    "shell-integration.sh" in first_line
                    or "ai-project-manager" in first_line
                ):
                    continue
