...
TIMESTAMP_PATTERN = re.compile(r"^[0-9]{8}-[0-9]{6}-[a-z]+-[a-f0-9]{6}$")
...
def _choose_permission_mode() -> dict | None:
    ...
    if choice in CLAUDE_PERMISSION_MODES:
        return dict(CLAUDE_PERMISSION_MODES[choice])
    print("无效选择，已取消。")
    return {"error": True}
...
def _choose_codex_mode() -> dict | None:
    ...
    if choice in CODEX_PROFILES:
        return dict(CODEX_PROFILES[choice])
    print("无效选择，已取消。")
    return {"error": True}
...
def _codex_preflight(project_dir: Path, profile: str) -> bool:
    ...
    try:
        ...
        if major < 0 or (major == 0 and minor < 144):
            print(
                f"\n错误: Codex 版本过低。当前: {ver}，需要: >= 0.144.0",
                file=sys.stderr,
            )
            return False
    except Exception:
    ...
    if "gpt-5.6-sol" not in content:
        print(
            f'\n错误: profile 文件缺少 model = "gpt-5.6-sol": {profile_file}',
            file=sys.stderr,
        )
        return False
    ...
    try:
        ...
        now = int(
            subprocess.check_output(["date", "+%s"], text=True, timeout=5).strip()
        )
        last_check = 0
        if preflight_file.is_file():
            first_line = (
                preflight_file.read_text(encoding="utf-8").split("\n")[0].strip()
            )
...
def recent_entries() -> list[str]:
    """Valid recent-project paths from the state file, most recent first."""
    RECENT_STATE_DIR.mkdir(parents=True, exist_ok=True)
...
def _fast_determine_project() -> Path | None:
    """Silent fast-path project detection (here-mode, preset, git toplevel).

    Returns ``None`` when the interactive menu is needed.  Prints nothing so
    the adaptive TUI can start from a clean screen.
    """
...
def determine_project() -> Path | None:
    fast = _fast_determine_project()
    if fast is not None:
        if os.environ.get("_CC_HERE_MODE") == "1":
            print(f"\ncc-here 模式: 使用当前目录 {fast}")
        else:
            preset = os.environ.get("_CC_PRESET_PROJECT", "")
            if preset and Path(preset).is_dir():
                print(f"\n使用预设项目: {fast}")
...
def main() -> int:
    ...
    # Interactive terminals get the content-aware adaptive TUI; scripts,
    # pipes, and tests keep the original line-prompt flow unchanged.
    from . import tui

    if tui.is_tui_session():
        return tui.tui_main()
    ...
    if not has_git(project_dir):
        ...
        if choice == "1":
            subprocess.run(
                ["git", "-C", str(project_dir), "init"], capture_output=True, timeout=10
            )
        elif choice == "2":
            pass
        elif choice.lower() == "q":
    ...
    while True:
        ...
        if provider == "o":
            ...
            print("║  工具: OpenAI Codex (GPT-5.6 Sol)")
            ...
            if perm_cfg.get("error"):
                return 1
            if perm_cfg.get("bypass") and not _confirm_remote(
