...
from .utils import (
    PROJECTS_ROOT, PROJECT_INDEX_FILE, ensure_dir, atomic_write_json, read_json,
    generate_session_id, generate_project_temp_name,
    generate_short_id, timestamp_iso, sanitize_project_title,
    compute_sha256, compute_text_hash
)
    ...
    if not projects and PROJECTS_ROOT.exists():
        for meta_path in PROJECTS_ROOT.glob("*/.ai-session/project.json"):
            ...
            if isinstance(meta, dict):
                projects.append(_project_choice(path, meta, {}, meta.get("session_id", "")))
...
PROJECT_DISCOVERY_MARKERS = (
    ".git", ".ai-session", "AGENTS.md", "CLAUDE.md", "README.md",
    "pyproject.toml", "package.json", "requirements.txt", "Cargo.toml",
    "go.mod", "CMakeLists.txt", "Makefile", "src", "app",
)
PROJECT_DISCOVERY_EXCLUDED = {"_Archive", "_Inbox", "trash", "cache", "__pycache__"}
PROJECT_DISCOVERY_SUFFIXES = {".py", ".js", ".ts", ".tsx", ".jsx", ".rs", ".go", ".java", ".c", ".cc", ".cpp", ".h", ".hpp", ".sh", ".md", ".toml", ".yaml", ".yml"}
...
def _has_project_evidence(path: Path) -> bool:
    ...
    for config_dir in (path / ".claude", path / ".codex"):
        if config_dir.is_dir() and any(item.is_file() for item in config_dir.rglob("*")):
            return True
    try:
        return any(item.is_file() and item.suffix.lower() in PROJECT_DISCOVERY_SUFFIXES for item in path.iterdir())
    except OSError:
        return False
...
def discover_project_directories() -> list[dict]:
    ...
    for entry in sorted(PROJECTS_ROOT.iterdir(), key=lambda p: p.name.casefold()):
        if entry.name.startswith(".") or entry.name in PROJECT_DISCOVERY_EXCLUDED or not entry.is_dir():
            continue
        path = entry.resolve()
        if path.parent != root or not _has_project_evidence(path):
            continue
        found.append({"project_id": None, "name": path.name, "path": str(path), "status": "unregistered", "tool": "—", "last_updated": ""})
    return found
...
def discover_selectable_projects() -> list[dict]:
    ...
    return registered + [item for item in discover_project_directories() if str(Path(item["path"]).resolve()) not in seen]
...
def _project_choice(path: Path, meta: dict, entry: dict, key: str) -> dict:
    return {
        ...
        "last_updated": (entry.get("last_updated") or meta.get("last_updated")
                         or meta.get("end_time") or meta.get("start_time") or ""),
    }
...
def print_project_choices(projects: list[dict]) -> None:
    ...
    for number, item in enumerate(projects, 1):
        ...
        print(f"    状态: {item['status']}  工具: {item['tool']}  last_updated: {item['last_updated'] or '-'}")
...
def create_project_meta(project_dir: Path, tool: str, session_id: str,
                        cli_session_id: str = "", model: str = "",
                        provider: str = "", permission_mode: str = "",
                        transcript_path: str = "", cli_path: str = "",
                        cli_version: str = "",
                        workflow_contract: dict | None = None) -> dict:
...
def create_session_meta(project_dir: Path, session_id: str, tool: str,
                        model: str = "", provider: str = "",
                        permission_mode: str = "", cli_path: str = "",
                        cli_version: str = "", cli_session_id: str = "",
...
def create_new_project(tool: str, model: str = "", provider: str = "",
                       permission_mode: str = "", cli_path: str = "",
                       cli_version: str = "",
                       workflow_contract_id: str | None = None) -> Path:
    ...
    if workflow_contract_id:
        try:
            ...
            if not matching:
                print(f"Error: unknown workflow contract: {workflow_contract_id}",
                      file=sys.stderr)
                raise SystemExit(1)
...
def _update_project_index(project_dir: Path, session_id: str, tool: str,
                          status: str = "running") -> None:
    """Update the global project index atomically."""
    from .utils import file_lock, GLOBAL_LOCK_FILE, STATE_DIR, atomic_write_json
...
def update_project_status(project_dir: Path, new_status: str,
                          session_id: str | None = None,
                          tool: str | None = None,
                          metadata: dict | None = None,
                          index_metadata: dict | None = None) -> bool:
    ...
    """
    from .utils import read_json, atomic_write_json, file_lock, GLOBAL_LOCK_FILE, STATE_DIR, ensure_dir, timestamp_iso
    ...
    if old_status != new_status and not validate_status_transition(old_status, new_status):
        ...
        try:
            ...
            with open(log_dir / "status-transitions.log", "a") as f:
                f.write(f"[{timestamp_iso()}] BLOCKED: {old_status} → {new_status} "
    ...
    session_meta_path = project_dir / ".ai-session" / "sessions" / session_id / "meta.json"
    old_session_meta = read_json(session_meta_path) if session_meta_path.exists() else None
