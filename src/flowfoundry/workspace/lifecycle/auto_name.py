...
def _get_markdown_section_paragraph(
    content: str, headings: tuple[str, ...]
) -> Optional[str]:
...
def _get_project_state_summary(project_dir: Path) -> Optional[str]:
    ...
    for state_file in [
        project_dir / ".ai" / "PROJECT_STATE.md",
...
def _get_source_code_clues(project_dir: Path) -> list[str]:
    ...
    for py_file in (
        list(project_dir.glob("*.py")) + list(project_dir.glob("src/**/*.py"))[:5]
    ):
    ...
    try:
        ...
        if result.returncode == 0 and result.stdout.strip():
            for line in result.stdout.strip().split("\n"):
                ...
                cleaned = re.sub(
                    r"^(ai-session|finalize|init|commit)[:\s]*",
                    "",
                    line,
                    flags=re.IGNORECASE,
                )
...
def analyze_project(project_dir: Path) -> dict:
            ...
            if manifest.exists():
                try:
                    ...
                    key_files = [
                        ...
                        if any(
                            kw in f.lower()
                            for kw in ["cc", "aiproj", "claude.md", "launcher", "hooks"]
                        )
                    ]
                    if key_files:
                        result["display_name"] = "AI 项目管理与启动器配置工作区"
                        result["summary"] = (
                            f"包含系统备份: {', '.join(Path(f).name for f in key_files[:5])}"
                        )
    ...
    if not result["display_name"] or result["display_name"] == name:
        if file_clues:
            result["display_name"] = _make_display_name_from_clues(
                file_clues, project_dir
            )
            result["naming_reason"] = (
                f"根据文件结构自动生成: {', '.join(file_clues[:3])}"
            )
...
def _classify_project_type(project_dir: Path, pj: dict) -> str:
    ...
    if any(
        kw in name
        for kw in ["claude", "codex", "deepseek", "ai-project", "cc-launcher"]
    ):
        return "developer-tooling"

    # Check file patterns
    has_py = (
        any(project_dir.glob("*.py"))
    ...
    has_theme = (project_dir / "theme").exists() or any(project_dir.glob("*/theme"))
    has_desktop_packaging = (project_dir / "packaging").exists() or (
        project_dir / "android"
    ).exists()
...
def _generate_dir_name(display_name: str, project_dir: Path) -> str:
    ...
    if segments:
        ...
        TRUNCATED_ENDINGS = {
            ...
            "launch",
            "laun",
            "lau",  # launcher (launch is a real word though)
            "workspa",
            "worksp",
            "works",  # workspace (works is a real word)
            "testin",
            "testi",  # testing
            "automate",
            "automat",  # automated/automation
            "mainten",
            "mainte",
            "maint",  # maintenance
            "orchestrat",
...
def _generate_name_candidates(project_dir: Path, analysis: dict) -> list[dict]:
    ...
    git_clues = _get_git_clues(project_dir)
    meaningful_git = [
        g for g in git_clues if not g.startswith("ai-session") and len(g) > 20
    ]
    if meaningful_git:
    ...
    if pj:
        ...
        if any(
            kw in str(project_dir) for kw in ["ai-project", "aiproj", "cc-launcher"]
        ):
            candidates.append(
                {
...
def _get_deployed_component_clues(project_dir: Path) -> list[str]:
    ...
    for f in project_dir.rglob("cc"):
        if f.is_file() and f.name == "cc":
            try:
                ...
                if (
                    "Claude Code Unified Launcher" in content
                    or "Claude / DeepSeek" in content
                ):
    ...
    if (project_dir / "ai_project_manager").exists() or list(
        project_dir.glob("**/ai_project_manager/__init__.py")
    ):
...
def _make_display_name_from_clues(clues: list[str], project_dir: Path) -> str:
    ...
    for c in clues:
        ...
        if " " not in c and (
            "_" in c or (c[0].isupper() and any(ch.islower() for ch in c))
        ):
            continue
        filtered.append(c)
...
def update_project_json(project_dir: Path, analysis: dict = None) -> dict:
    ...
    result = {
        ...
        "project_type": analysis.get(
            "project_type", existing.get("project_type", "unknown")
        ),
        "suggested_directory_name": analysis.get(
            "suggested_directory_name", project_dir.name
        ),
        "naming_reason": analysis.get("naming_reason", ""),
        "is_placeholder": analysis.get(
            "is_placeholder", is_placeholder_name(project_dir.name)
        ),
...
def safe_rename_project(
    project_dir: Path, new_name: str, dry_run: bool = False
) -> dict:
    ...
    if not dry_run:
        try:
            ...
            proc_result = subprocess.run(
                ["pgrep", "-f", f"cd {project_dir}"], capture_output=True, timeout=5
            )
