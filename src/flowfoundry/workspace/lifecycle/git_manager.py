...
def _git_bytes(p, a, env=None, timeout=30):
    return subprocess.run(
        [b"git", b"-C", os.fsencode(p), *[os.fsencode(x) for x in a]],
        capture_output=True,
        timeout=timeout,
        env=env,
    )
...
def _decode_paths(b):
    ...
    if not b.endswith(b"\0"):
        raise ValueError("non-NUL paths")
...
def _skip(x):
    ...
    return (
        x == ".env"
        or n.startswith(".env.")
        or tuple(p[:2]) == (".ai-session", "private")
        or "secrets" in p
        or "__pycache__" in p
        or n.endswith(
            (
...
def _special(root):
    ...
    while todo:
        ...
        for e in es:
            ...
            elif not stat.S_ISREG(m) and not stat.S_ISLNK(m):
                out.append(rel)
    return out
...
def _index(p):
    ...
    t = _git_bytes(p, ["rev-parse", "--show-toplevel"])
    g = _git_bytes(p, ["rev-parse", "--absolute-git-dir"])
    i = _git_bytes(p, ["rev-parse", "--git-path", "index"])
    if any(x.returncode for x in (t, g, i)):
        raise RuntimeError("index resolution")
    t = Path(os.fsdecode(t.stdout.rstrip())).resolve()
    g = Path(os.fsdecode(g.stdout.rstrip())).resolve()
    raw = Path(os.fsdecode(i.stdout.rstrip()))
    out = (raw if raw.is_absolute() else t / raw).resolve(strict=False)
    if out.parent != g and g not in out.parents:
        raise RuntimeError("index escape")
    return out
...
def git_add_all_safe(project_dir: Path, lock_timeout: float = 30) -> dict:
    ...
    try:
        with file_lock(root / ".ai-session/locks/git-index.lock", timeout=lock_timeout):
            ...
            if q.returncode == 1:
                return _stage_result(blocked=["pre-existing-staged-changes"])
            if q.returncode:
                return _stage_result(errors=["git-diff-cached-check-failed"])
            ix = _index(root)
            index_path = ix
            m = _git_bytes(root, ["ls-files", "-m", "-d", "-z"])
            o = _git_bytes(root, ["ls-files", "--others", "--exclude-standard", "-z"])
            if m.returncode or o.returncode:
                return _stage_result(errors=["git-ls-files-failed"])
            candidates = list(
                dict.fromkeys(
                    _decode_paths(m.stdout) + _decode_paths(o.stdout) + _special(root)
                )
            )
            if not candidates:
                return _stage_result(ok=True, nothing_to_commit=True)
            add = []
            skip = []
            block = []
            errors = []
            for rel in candidates:
                ...
                try:
                    ...
                    if parent != root and root not in parent.parents:
                        block.append(f"{rel}:path-escape")
                        continue
                    st = p.lstat()
                except FileNotFoundError:
                    z = _git_bytes(root, ["ls-files", "--error-unmatch", "--", rel])
                    (add if not z.returncode else errors).append(
                        rel if not z.returncode else f"{rel}:lstat-failed"
                    )
                    continue
                except OSError:
                    errors.append(f"{rel}:lstat-failed")
                    continue
                if stat.S_ISLNK(st.st_mode):
                    block.append(f"{rel}:symlink")
                    continue
                if not stat.S_ISREG(st.st_mode):
                    block.append(f"{rel}:non-regular-file")
                    continue
                try:
                    if p.stat().st_size > MAX_STAGE_FILE_SIZE:
                        block.append(f"{rel}:too-large")
                        continue
                    b = p.read_bytes()
                    if b"\0" in b:
                        block.append(f"{rel}:binary")
                        continue
                    if scan_for_secrets(b.decode("utf-8")):
                        block.append(f"{rel}:secret-detected")
                        continue
                    add.append(rel)
                except UnicodeDecodeError:
                    block.append(f"{rel}:binary")
                except (OSError, ValueError):
                    errors.append(f"{rel}:content-scan-failed")
            if block or errors:
                return _stage_result(skipped=skip, blocked=block, errors=errors)
            if not add:
                return _stage_result(ok=True, skipped=skip, nothing_to_commit=True)
            ix.parent.mkdir(parents=True, exist_ok=True)
            fd, n = tempfile.mkstemp(prefix=".aiproj-index-", dir=ix.parent)
            os.close(fd)
            os.unlink(n)
            tmp = Path(n)
            if ix.exists():
                ...
                if _git_bytes(root, ["read-tree", "--empty"], env=env).returncode:
                    return _stage_result(
                        skipped=skip, errors=["temp-index-init-failed"]
                    )
            env = os.environ.copy()
            env["GIT_INDEX_FILE"] = str(tmp)
            if _git_bytes(root, ["add", "--", *add], env=env).returncode:
                return _stage_result(skipped=skip, errors=["git-add-failed"])
            d = _git_bytes(root, ["diff", "--cached", "--binary"], env=env)
            if d.returncode:
                return _stage_result(skipped=skip, errors=["cached-diff-scan-failed"])
            try:
                text = d.stdout.decode("utf-8")
            except UnicodeDecodeError:
                return _stage_result(skipped=skip, errors=["cached-diff-decode-failed"])
            if scan_for_secrets(text):
                return _stage_result(
                    skipped=skip, blocked=["cached-diff:secret-detected"]
                )
            with tmp.open("rb") as f:
                os.fsync(f.fileno())
            original_bytes = ix.read_bytes() if ix.exists() else None
            original_mode = stat.S_IMODE(ix.stat().st_mode) if ix.exists() else None
            os.replace(tmp, ix)
            tmp = None
            replaced = True
            if original_mode is not None:
                os.chmod(ix, original_mode)
        ...
        return _stage_result(errors=["git-index-lock-timeout"])
            ...
            try:
                if original_bytes is None:
                    ...
                    fd, name = tempfile.mkstemp(
                        prefix=".aiproj-restore-", dir=index_path.parent
                    )
                ...
                return _stage_result(
                    errors=[
                        f"CRITICAL-index-restore-failed:{type(restore_error).__name__}"
                    ]
                )
