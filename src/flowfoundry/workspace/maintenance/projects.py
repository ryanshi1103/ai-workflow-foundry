...
def _run_git(
    project_dir: Path, args: list[str], timeout: int = 60
    ...
    for entry in load_managed_projects():
        ...
        try:
            ...
            if dirty.returncode != 0:
                results.append(
                    {**result, "status": "error", "detail": dirty.stderr.strip()}
                )
                continue
            ...
            branch = _run_git(
                project_dir, ["symbolic-ref", "--quiet", "--short", "HEAD"]
            )
            ...
            if counts.returncode != 0:
                results.append(
                    {**result, "status": "error", "detail": counts.stderr.strip()}
                )
                continue
            ...
            if ahead:
                ...
                results.append(
                    {**result, "status": status, "ahead": ahead, "behind": behind}
                )
                continue
            if not behind:
            ...
            if merged.returncode == 0:
                ...
                results.append(
                    {**result, "status": "merge-error", "detail": merged.stderr.strip()}
                )
...
def _has_source_content(project_dir: Path) -> bool:
    ...
    try:
        if any(
            p.is_file() and p.suffix.lower() in source_suffixes
...
def _has_bounded_content(
    project_dir: Path, suffixes: set[str], content_dirs: set[str]
) -> bool:
    """Find deliverables at the root or one level inside known content dirs."""
    try:
        if any(
            p.is_file() and p.suffix.lower() in suffixes for p in project_dir.iterdir()
        ):
            return True
    except OSError:
        return False
...
def _has_nested_project(project_dir: Path) -> bool:
    ...
    for child in children:
        if not (child / "README.md").is_file():
            continue
        if _has_source_content(child) or any(
            (child / marker).exists()
            for marker in (
                "pyproject.toml",
                "package.json",
...
def _is_session_only_project(project_dir: Path) -> bool:
    ...
    for f in project_dir.rglob("*"):
        ...
        in_session_dir = any(
            rel.startswith(d + "/") or rel == d for d in session_artifact_dirs
        )
...
def _similarity_score(path1: Path, path2: Path) -> float:
    ...
    pj1 = read_json(path1 / ".ai-session" / "project.json") or read_json(
        path1 / ".ai" / "project.json"
    )
    pj2 = read_json(path2 / ".ai-session" / "project.json") or read_json(
        path2 / ".ai" / "project.json"
    )
    if pj1 and pj2:
    ...
    try:
        r1 = subprocess.run(
            ["git", "-C", str(path1), "remote", "get-url", "origin"],
            capture_output=True,
            text=True,
            timeout=5,
        )
        r2 = subprocess.run(
            ["git", "-C", str(path2), "remote", "get-url", "origin"],
            capture_output=True,
            text=True,
            timeout=5,
        )
...
def quarantine_project(project_dir: Path, reason: str, dry_run: bool = False) -> dict:
...
def restore_from_quarantine(quarantine_path: str) -> dict:
    ...
    if not original_path:
        return {
            "success": False,
            "error": "Original path not found in quarantine index",
        }
...
def quick_maintenance(dry_run: bool = False) -> dict:
    ...
    log_maintenance(
        f"Quick maintenance completed. {len(report['name_suggestions'])} name suggestions."
    )
    return report
            ...
            if (
                ...
                if not dry_run and not proj["is_protected"]:
                    result = safe_rename_project(
                        Path(proj["path"]), proj["suggested_name"], dry_run=dry_run
                    )
                    if result["success"]:
        ...
        for proj in all_projects:
            if proj["category"] == "D" and not proj["is_protected"]:
                ...
                if entry.exists() and entry.parent == PROJECTS_ROOT.resolve():
                    ...
                    if not dry_run:
                        quarantine_project(
                            entry, "纯 AI 会话记录，无独立交付成果", dry_run=dry_run
                        )
    ...
    for entry in sorted(PROJECTS_ROOT.iterdir()):
        ...
        proj = {
            ...
            "suggested_action": _suggest_action(
                category, is_prot, is_placeholder_name(entry.name)
            ),
...
def run_maintenance(mode: str, dry_run: bool = False) -> dict:
    ...
    if mode == "quick":
        ...
        config = load_config()
        if (
            config.get("AUTO_PURGE_AFTER_RETENTION", "true").lower() != "true"
            and not dry_run
        ):
