...
from .utils import (
    ...
    file_lock, generate_short_id, sanitize_project_title,
    INTERNAL_ENV_VAR,
)
from .transcript_claude import (
    parse_claude_transcript, extract_conversation_markdown as claude_md,
...
from .transcript_codex import (
    parse_codex_transcript, extract_conversation_markdown as codex_md,
...
from .redact import redact_jsonl, scan_for_secrets
from .git_manager import (
    git_status, git_add_all_safe,
    git_commit, build_commit_message,
)
from .project import update_project_status


def finalize_session(project_dir: Path, session_id: str | None = None,
                     tool: str = "claude", use_ai: bool = False) -> dict:
    ...
    try:
        ...
        result["error"] = "Could not acquire finalize lock (another finalize in progress?)"
        return result


def _finalize_impl(project_dir: Path, session_id: str | None,
                   tool: str, use_ai: bool, result: dict) -> dict:
    ...
    if meta.get("status") in ("completed", "interrupted"):
        private_final = read_json(
            project_dir / ".ai-session" / "private" /
            (session_id or meta.get("session_id", "")) / "finalization.json"
        ) or {}
        result["success"] = True
    ...
    session_meta = read_json(session_dir / "meta.json") if session_dir.exists() else None
    ...
    try:
        ...
        _update_session_docs(project_dir, session_dir, session_id, events,
                             actual_tool, goal, accomplishments, decisions, final_status)

        # Step 9: Merge into project-level docs
        _merge_project_docs(project_dir, session_id, goal, accomplishments,
                            decisions, final_status, actual_tool)
        ...
        if not update_project_status(
            project_dir, final_status, session_id=session_id, tool=actual_tool,
        ...
        commit_result = _do_git_commit(project_dir, session_id, actual_tool,
                                      meta, goal, accomplishments, decisions,
                                      final_status, session_meta)
        final_commit=commit_result.get("commit")
        if not commit_result.get("success"):
            _write_staging_problems(project_dir,session_id,commit_result.get("stage_result",{}));update_project_status(project_dir,"failed",session_id=session_id,tool=actual_tool)
            result.update(status="failed",error=commit_result.get("error","safe staging failed"),stage_result=commit_result.get("stage_result"));return result
        ...
        failure_fields = {
            ...
            "summary_mode": "deterministic",
            "first_prompt_hash": current_session.get("first_prompt_hash") or meta.get("first_prompt_hash"),
            "transcript_hash": current_session.get("transcript_hash") or meta.get("transcript_hash"),
            "redaction_applied": bool(current_session.get("redaction_applied", False)),
            "finalize_attempts": max(int(meta.get("finalize_attempts", 0)), int(current_session.get("finalize_attempts", 0))) + 1,
            "last_error": str(e),
            "final_commit": None,
        }
        update_project_status(project_dir, "failed", session_id=session_id,
                              tool=actual_tool, metadata=failure_fields)
        result["status"] = "failed"
        return result


def _record_final_commit(project_dir: Path, session_id: str, tool: str,
                         final_commit: str | None) -> None:
    """Record a commit id without modifying files contained in that commit."""
    from .utils import STATE_DIR, GLOBAL_LOCK_FILE
    private_dir = project_dir / ".ai-session" / "private" / session_id
    ensure_dir(private_dir, 0o700)
    atomic_write_json(private_dir / "finalization.json", {
        "session_id": session_id,
        "status": "completed",
    ...
    with file_lock(GLOBAL_LOCK_FILE):
        ...
        entry.update({"path": str(project_dir), "tool": tool,
                      "status": "completed", "final_commit": final_commit,
                      "last_updated": timestamp_iso()})
        atomic_write_json(index_path, index)


def _final_sync_transcript(project_dir: Path, session_dir: Path,
                           session_meta: dict | None, tool: str) -> None:
...
def _update_transcript_hash(session_dir: Path, session_meta: dict | None) -> None:
    ...
    raw_dest = session_dir.parent.parent / "private" / session_dir.name / "transcript.raw.jsonl"
...
def _parse_transcript(session_dir: Path, tool: str) -> list[dict]:
    ...
    private_raw = session_dir.parent.parent / "private" / session_dir.name / "transcript.raw.jsonl"
...
def _ensure_redacted_transcript(project_dir: Path, session_dir: Path, tool: str) -> None:
    ...
    private_raw = project_dir / ".ai-session" / "private" / session_dir.name / "transcript.raw.jsonl"
...
def _generate_conversation_md(project_dir: Path, session_dir: Path,
                               events: list[dict], tool: str) -> None:
...
def _update_session_docs(project_dir: Path, session_dir: Path, session_id: str,
                          events: list[dict], tool: str, goal: str,
                          accomplishments: list[str], decisions: list[str],
                          status: str) -> None:
...
def _merge_project_docs(project_dir: Path, session_id: str, goal: str,
                         accomplishments: list[str], decisions: list[str],
...
def _merge_summary(docs_dir: Path, session_id: str, goal: str,
                   accomplishments: list[str], status: str, ts: str) -> None:
    ...
    if session_marker in content:
        ...
        content = content[:start] + entry_text.lstrip("\n") + "\n" + content[end:].lstrip("\n")
    else:
...
def _merge_decisions(docs_dir: Path, session_id: str, decisions: list[str], ts: str) -> None:
...
def _merge_tasks(docs_dir: Path, session_id: str, accomplishments: list[str], ts: str) -> None:
    ...
    insert_pos = content.find("\n", completed_section) + 1
    new_tasks = "\n".join(
        [f"<!-- session:{session_id} -->\n- [x] {acc}" for acc in accomplishments[:20]]
    ) + "\n"
...
def _merge_files_changed(docs_dir: Path, project_dir: Path, session_id: str, ts: str) -> None:
    ...
    all_files = (status["staged_files"] + status["changed_files"] +
...
def _do_git_commit(project_dir: Path, session_id: str, tool: str,
                   meta: dict, goal: str, accomplishments: list[str],
                   decisions: list[str], status: str,
                   session_meta: dict | None) -> dict:
    """Stage files and create git commit."""
    gs = git_status(project_dir)
    if not gs["is_repo"]:
        return {"success":True,"commit":None,"stage_result":{"ok":True,"added":[],"skipped":[],"blocked":[],"errors":[],"index_committed":False,"nothing_to_commit":True}}

    # Stage all safe files
    stage_result=git_add_all_safe(project_dir)
    safe=stage_result.get("ok") is True and stage_result.get("index_committed") is True and stage_result.get("nothing_to_commit") is False and not stage_result.get("blocked") and not stage_result.get("errors")
    if stage_result.get("ok") is True and stage_result.get("nothing_to_commit") is True:return {"success":True,"commit":None,"stage_result":stage_result}
    if not safe:return {"success":False,"commit":None,"stage_result":stage_result,"error":"safe staging blocked or failed"}
    ...
    subject, body = build_commit_message(
        ...
        transcript_hash=transcript_hash,
        redaction=session_meta.get("redaction_applied", False) if session_meta else False,
        summary_mode=meta.get("summary_mode", "deterministic"),
    )

    h=git_commit(project_dir,subject,body)
    if not h:return {"success":False,"commit":None,"stage_result":stage_result,"error":"git commit failed"}
    return {"success":True,"commit":h,"stage_result":stage_result}

def _write_staging_problems(project_dir:Path,session_id:str,stage_result:dict)->None:
 p=project_dir/"docs/problems-and-solutions.md";ensure_dir(p.parent)
 if not p.exists():p.write_text("# Problems and Solutions\n\n",encoding="utf-8")
 lines=[f"\n## Safe staging failure ({session_id})",""]
 for c in("blocked","errors"):lines.extend(f"- {c}: {x}" for x in stage_result.get(c,[]))
...
def _rename_project_if_possible(project_dir: Path, first_prompt: str) -> None:
    ...
    from .project import rename_project, update_project_status
