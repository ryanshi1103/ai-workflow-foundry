...
from .utils import (
    ...
    timestamp_iso, INTERNAL_ENV_VAR, find_real_executable,
)
from .project import (
    create_new_project, create_session_meta, read_project_meta,
    _update_project_index,
)
from .git_manager import git_init, ensure_git_identity
from .finalize import finalize_session
from .recovery import auto_recover_on_startup


def launch_new(tool: str, cli_path: str | None = None,
               extra_args: list[str] | None = None,
               env: dict | None = None,
               model: str = "", provider: str = "",
               permission_mode: str = "",
               workflow_contract_id: str | None = None) -> int:
    ...
    **IMPORTANT**: If CC_ACTIVE_PROJECT is set in the environment, this
    ...
    if cc_active:
        ...
        if cc_project.exists():
            print(f"  AI Project Manager: Using cc-selected project {cc_project}",
                  file=sys.stderr)
            return launch_here(
                tool=tool,
    ...
    try:
        ...
        print(f"  AI Project Manager: Created project {project_dir.name}", file=sys.stderr)
...
def launch_here(tool: str, project_dir: Path | None = None,
    ...
    **IMPORTANT**: CC_ACTIVE_PROJECT is the single source of truth.
    ...
    if cc_active:
        ...
        if cc_project.exists():
            ...
            print(f"  AI Project Manager: Using CC_ACTIVE_PROJECT={project_dir}",
                  file=sys.stderr)
    ...
    if not meta:
        ...
        git_init(project_dir)
        ensure_git_identity(project_dir)
        from .project import create_project_structure, create_project_meta
...
def _safe_finalize(project_dir: Path, tool: str) -> bool:
    ...
    try:
        ...
        if not r.get("success"):print(f"Warning: finalize failed: {r.get('error','unknown error')}",file=sys.stderr);return False
        return True
    except Exception as e:
...
def is_non_interactive_args(tool: str, args: list[str]) -> bool:
    ...
    if tool == "claude":
        non_interactive = {
            '-p', '--print', '-c', '--continue', '-r', '--resume',
            '--resume', 'agents', 'auth', 'doctor', 'gateway', 'install',
            'mcp', 'plugin', 'plugins', 'project', 'setup-token',
            'update', 'upgrade', 'ultrareview',
        }
    ...
    if tool == "codex":
        non_interactive = {
            ...
            'update', 'doctor', 'sandbox', 'debug', 'apply', 'a',
            'resume', 'archive', 'delete', 'unarchive', 'fork',
