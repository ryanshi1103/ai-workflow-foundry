...
PERSONAL_AI_FIXTURE = ROOT / "examples/personal-ai/personal-ai-manager.json"
PERSONAL_AI_EXPECTED_PLAN = (
    ROOT / "docs/assets/demos/personal-ai-manager-plan.json"
)
...
class TeamCliTests(unittest.TestCase):
    ...
    def test_personal_ai_demo_plan_matches_committed_fixture(self) -> None:
        result, output, error = self.call("team", "plan", str(PERSONAL_AI_FIXTURE))
        self.assertEqual((result, error), (0, ""))
        expected = json.loads(PERSONAL_AI_EXPECTED_PLAN.read_text(encoding="utf-8"))
