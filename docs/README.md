# FlowFoundry Documentation

Use [AUTHORITATIVE_DOCUMENT_INDEX.md](AUTHORITATIVE_DOCUMENT_INDEX.md) when two
documents appear to cover the same topic. It defines the canonical product,
architecture, roadmap, and release sources.

For audience-based navigation, presentation assets, designed/future labels, and
superseded names, use the
[Authoritative Document Map](AUTHORITATIVE_DOCUMENT_MAP.md).

## Start here

1. [Root README](../README.md) — understand FlowFoundry in ten seconds.
2. [First External Alpha User Guide](ALPHA_USER_GUIDE.md) — install and reach
   the human approval boundary in thirty minutes.
3. [Current Status](CURRENT_STATUS.md) and [Limitations](LIMITATIONS.md) — see
   what is implemented, experimental, designed, future, or blocked.
4. [GitHub Release Assistant](demos/github-release-assistant.md) — reproduce the
   canonical first demo.

## Product and engineering

- [Product Architecture](FLOWFOUNDRY_PRODUCT_ARCHITECTURE.md) — canonical
  product layers and trust boundaries.
- [Technical Architecture](ARCHITECTURE.md) — module-level implementation.
- [Product Roadmap](PRODUCT_ROADMAP.md) — canonical current and future stages.
- [Vision](VISION.md) — long-term motivation, not shipped capability evidence.

## Install and operate

- [Installation](INSTALLATION.md)
- [Troubleshooting](TROUBLESHOOTING.md)
- [FAQ](FAQ.md)
- [Limitations](LIMITATIONS.md)
- [Operator Guide](../MULTI_AGENT_OPERATOR_GUIDE.md)
- [Security Policy](../SECURITY.md)

## Release

- [Final Candidate Checklist](FINAL_CANDIDATE_CHECKLIST.md) — canonical local
  assembly and reviewer handoff.
- [Alpha Release Checklist](ALPHA_RELEASE_CHECKLIST.md) — canonical GO/NO-GO
  gate.
- [Final Candidate Report](../FINAL_CANDIDATE_REPORT.md) — exact included and
  excluded change inventory, validation, and blockers.
- [Final Release Status](FINAL_RELEASE_STATUS.md) — current evidence snapshot.
- [Final Release Report](../FINAL_RELEASE_REPORT.md) — detailed baseline
  evidence and historical-artifact warning.
- [Release Day Runbook](RELEASE_DAY_RUNBOOK.md) — authorized operator sequence.
- [Launch Scorecard](LAUNCH_SCORECARD.md) — current numeric readiness.
- [Launch Package](LAUNCH_PACKAGE.md) — draft public release copy.
- [Sanitization Report](SANITIZATION_REPORT.md)
- [License Decision](LICENSE_DECISION.md)

## Demo and visual evidence

- [Demo Recording Checklist](DEMO_RECORDING_CHECKLIST.md)
- [Demo Asset Checklist](DEMO_ASSET_CHECKLIST.md)
- [Demo Index](demos/README.md)
- [90-second Flagship Script](demos/90_SECOND_DEMO_SCRIPT.md)
- [Launch Story](demos/LAUNCH_STORY.md)
- [Visual Product Story](VISUAL_STORY.md)
- [Visual Design System](VISUAL_DESIGN.md)
- [Overnight Product Audit](OVERNIGHT_PRODUCT_AUDIT.md)

No concept image or rendered preview is runtime proof. Actual screenshots must
identify the exact candidate SHA and execution mode.

## Users and contributors

- [Contributing](CONTRIBUTING.md)
- [Five Good First Issues](GOOD_FIRST_ISSUES.md)
- [Contributor Journey](CONTRIBUTOR_JOURNEY.md)
- [Community Operating Model](COMMUNITY_OPERATING_MODEL.md)
- [First 10 Users Risk Simulation](FIRST_10_USERS_VALIDATION.md) — simulation,
  not external evidence.
- [First 100 Users Experiment](FIRST_100_USERS_EXPERIMENT.md)

## Positioning and trust

- [Final Competitive Positioning](COMPETITIVE_POSITIONING_FINAL.md)
- [Detailed Positioning Reference](POSITIONING.md)
- [Trust Audit](TRUST_AUDIT.md)
- [Public Documentation Audit](PUBLIC_DOCUMENTATION_AUDIT.md)
- [Final Product Review](PRODUCT_FINAL_REVIEW.md)
- [GitHub Trusted-user Strategy](GITHUB_STAR_STRATEGY.md)
- [Public Website Structure](WEBSITE_STRUCTURE.md)
- [Website Content](WEBSITE_CONTENT.md)
- [Marketing Launch Plan](MARKETING_LAUNCH_PLAN.md)

## Designed and future — not shipped

- [Mobile AI Command Center](MOBILE_AI_COMMAND_CENTER.md)
- [Mobile PWA MVP](MOBILE_PWA_MVP.md)
- [Mobile Security Model](MOBILE_SECURITY_MODEL.md)
- [Remote Agent Architecture](REMOTE_AGENT_ARCHITECTURE.md)
- [Personal AI Manager](PERSONAL_AI_MANAGER.md)
- [Personal AI OS Strategy](PERSONAL_AI_OS_STRATEGY.md)

These documents describe product direction only. They do not prove a mobile
application, personal-memory system, or Personal AI OS exists.
