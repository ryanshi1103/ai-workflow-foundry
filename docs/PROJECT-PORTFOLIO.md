...
本清单最初基于 `/home/ryan/Projects` 下 13 个活跃顶层目录的 README、交付物、
源码结构、Git 状态和现有项目索引整理；2026-08-03 又复核了全部活跃项目，并
增加旗舰及公开快照的规范本地克隆。目录名统一建议使用小写 `kebab-case`；产品
显示名可以保留中文。隔离区不计入活跃项目。

`~/Projects` 顶层还保留公开发布与集成所需的规范 Git 克隆，但它们不等同于新增
业务项目。`cc` 主选择器只显示逻辑主项目；公开派生仓库、个人主页仓库和遗留复核
目录进入 `m` 托管子列表。托管 Git 仓库由每周维护在工作区干净且可快进时自动
更新，遇到本地改动、领先或分叉则停止并报告。
...
| AI 工作区底座 | `ai-workspace-manager` | 物理纳入 `core/`，保留原提交历史 |
| 媒体 AI | `confera-media-skills`、私有会映仓库 | 公开 Skill 物理纳入 `components/`；完整产品保持私有 |
| 客户反馈智能 | `feedback-analysis-system` | 公开应用物理纳入 `applications/`；私有数据源独立保留 |
| 文档自动化 | `print-ready-nameplate-generator` | 通用工作流物理纳入 `workflows/`；真实名单不纳入 |
| 设备、数据、系统 | Android 工具、GRUB 主题 | 专项产品；不用于定义 AI 旗舰边界 |

AI 工作流的公开、脱敏、可复用代码统一进入 FlowFoundry monorepo；跨类别项目仍只
提炼经验证的优秀模式，例如人工审核、不可变修订、哈希与原子写、备份与回滚，
不把真实数据、商业发行工程或无关工具硬合并进去。
